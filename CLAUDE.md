...
- **⭐ The site is mid-redesign, and two visual systems coexist. Read
  ...
  - **NEW system** (`design-demos/redesign.html` + `redesign2.html` + `redesign3.html` +
    ...
    page: `/` (home), `/donate`, `/membership`, `/letters-to-santa`, `/santas-knights`, `/contact`,
...
**The letter pile is public; the two write actions are not.** Anyone — signed out included — can read the pile on `/letters-to-santa`, so a stranger can see a real wish before deciding to make an account. **Claiming** a letter requires an account (the claim ties the gift to a donor, enables the acknowledgment, and blocks a guardian from gifting their own child's letter — `app/letters-to-santa/give/actions.ts`). **Submitting** a letter requires an account (the row carries `guardian_user_id`, which powers `/members` → "My letters"). Don't gate reading, and don't ungate either write.
