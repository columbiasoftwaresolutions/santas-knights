...
> ⚠️ **Build boundary revised.** This repo (`santasknights.org`) builds the nonprofit pages, Letters, member accounts, gallery, donate/membership, and partner content. Gladiators NYC public class marketing pages now live on the separate **`gladiators.nyc`** site at `/classes` and `/classes/[slug]`, alongside booking, waivers + media consent, instructor check-in, XP, participant dashboards, direct-to-Supabase training-video upload/management, training admin + CSV export, Shop, and Armory. `santasknights.org` redirects/cross-links to `gladiators.nyc` for classes, booking, shop purchases, and armor rentals. **Shared identity for now:** one login across both sites (a single Supabase Auth + `profiles` + `app_role`).
...
- **This site = `santasknights.org`** — the Santa's Knights nonprofit: nonprofit pages, the Letters to Santa program, member accounts, gallery, donate/membership, and partner content. Gladiators NYC class marketing, operational training tracker, training videos, Shop, and Armory live on `gladiators.nyc`. **Shared identity for now** — one login (Supabase Auth + `profiles` + `app_role`) across both sites.
- `SUPABASE_SECRET_KEY` server-only.

---

## Site Architecture & Nonprofit-vs-Commercial Split
